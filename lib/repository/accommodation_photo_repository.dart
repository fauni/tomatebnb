...
Future<ApiResponse<AccommodationPhotoResponseModel>> delete(int photoId) async {
    try{
      final prefs = await SharedPreferences.getInstance();
      String token = prefs.getString("token")??"";
      final response = await http.delete(
        Uri.parse('$_baseUrl/v1/accommodation_photos/$photoId'),
        headers: <String, String>{
 ...
 Future<ApiResponse<AccommodationPhotoResponseModel>> update(int id, AccommodationPhotoResponseModel accommodation) async {
    try{
      final prefs = await SharedPreferences.getInstance();
      String token = prefs.getString("token")??"";
      final response = await http.put(
        Uri.parse('$_baseUrl/v1/accommodation_photos/$id'),
        headers: <String, String>{
          'Content-Type': 'application/json',
          'Accept' : 'application/json',
          'Authorization':'Bearer $token',
...
Future<ApiResponse<AccommodationPhotoResponseModel>> getMainByAccommodation(int accommodationId) async {
    try{
      final prefs = await SharedPreferences.getInstance();
      String token = prefs.getString("token")??"";
      final response = await http.get(
        Uri.parse('$_baseUrl/v1/accommodation_photos/main/$accommodationId/accommodation'),
        headers: <String, String>{
          'Content-Type': 'application/json',
          'Accept' : 'application/json',
          'Authorization':'Bearer $token',
